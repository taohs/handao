...
//todo 将用户名称和车辆名称现实于报告顶部，确认车辆信息以便查看
     ...
     * @return bool|void
     */
    public function confirmAuth($oid){

        $auth = $this->session->get('auth');
        $worker = $this->session->get('worker');
        if(!$auth && !$worker){
            return false;
        }
        ...
        $orderUser = HdOrder::findFirst(array(
            'conditions'=>'id=:id:',
            'bind'=>array('id'=>$oid)
        ));
        if($orderUser && $orderUser->user_id == $auth->id){
            return $auth;
        }
        return false;
