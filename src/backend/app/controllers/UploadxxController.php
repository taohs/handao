 ...
 * @package PHP
 */
class UploadxxController extends ControllerBase
{
...
//        $tree = array();
    ...
    function listAction(){
        $name = APP_PUBLIC . '/uploads/files/' . '0bb3f486e827ab4d4aae39c216a8c9c6'. '.' . 'csv';
        $name = '/Users/macbook/Apps/02handao/docs/data/csv/汽车项目基础更新资料(更新20151018)/汽车基础数据（奥迪进口）-表格 1.csv';// . '/uploads/files/' . '0bb3f486e827ab4d4aae39c216a8c9c6'. '.' . 'csv';
        $name = '/Users/macbook/Apps/02handao/docs/data/csv/汽车项目基础资料(更新20151018)/汽车基础数据（奥迪进口）-表格 1.csv';// . '/uploads/files/' . '0bb3f486e827ab4d4aae39c216a8c9c6'. '.' . 'csv';
//        $name = '/Users/macbook/Apps/02handao/docs/data/csv/汽车基础数据（奥迪一汽）-表格 1.csv';// . '/uploads/files/' . '0bb3f486e827ab4d4aae39c216a8c9c6'. '.' . 'csv';
        $dir = '/Users/macbook/Apps/02handao/docs/data/csv/汽车项目基础资料(更新20151018)';
        $dirModel = new DirectoryIterator($dir);
        foreach($dirModel as $fileinfo){
            if(!$fileinfo->isDir()){
...
//        if (is_null($brands) or empty(trim($brands))) {
            return self::$brands;
        } else {
            //查询 没有就新增
...
//                echo 1;
    ...
    {
//        if (!empty($products)) {
        if (!empty(trim($products)) && trim($products)!='暂无配件' ) {
            $productsNameArray = explode('￥', $products);
            if(count($productsNameArray)<>2){
                return false;
            }
